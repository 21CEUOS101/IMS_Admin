...
import { getW2WOrderByWarehouseId } from "../../Services/W2WOrderService";
...
const W2WOrderByWManager = () => {
    const id = useParams().id;
  const [data, setData] = useState([]);

  const getAllOrders = async () => {
    const response = await getW2WOrderByWarehouseId(id)
      .then((response) => {
        setData(response);
      })
      .catch((err) => {
...
export default W2WOrderByWManager;
